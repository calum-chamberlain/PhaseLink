"""
Tools for building a synthetic dataset of arrival times for training purposes.
"""
...
def output_thread(out_q: mp.Queue, training_dataset_x: str, training_dataset_y: str):
    """ Wouldn't a doc-string be nice?! """
    none_count = 0
    X = []
    Y = []
    while True:
    ...
    np.save(training_dataset_x, X)
    np.save(training_dataset_y, Y)

    return


def generate_phases(
    in_q, out_q, x_min, x_max, y_min, y_max, sncl_idx, stla, stlo, phasemap, tt_p, tt_s
):
    """ What is the point of this? Doc-strings are gooooood. """
    ...
    while True:
        ...
        if n_fake > 0:
            ...
            if np.random.rand() < 0.5:
                idx = np.ones(n_fake, dtype=np.int) * np.random.choice(
                    np.arange(stlo.size)
                )
            else:
                idx = np.random.choice(np.arange(stlo.size), size=n_fake, replace=True)
            phase_labels = phasemap[idx]
            lons = stlo[idx]
            lats = stla[idx]
            tt = np.random.uniform(0, t_max, size=n_fake)
            X.append(
                np.column_stack((lons, lats, tt, phase_labels, np.ones(lons.shape[0])))
            )
            Y.append(np.zeros(lats.size))
            R.append(np.zeros(lats.size))

        Y0 = np.random.uniform(bounds_scaler * y_min, bounds_scaler * y_max)
        X0 = np.random.uniform(bounds_scaler * x_min, bounds_scaler * x_max)
        ...
        for i in range(n_eve):
            ...
            max_dist = np.random.uniform(
                params["min_hypo_dist"], params["max_hypo_dist"]
            )
...
def build_station_map(params):
    ...
    with open(params["station_file"], "r") as f:
        for line in f:
            ...
            stla = float(lat)
            stlo = float(lon)
            X0 = gps2dist_azimuth(lat0, stlo, lat0, lon0)[0] / 1000.0
            if stlo < lon0:
                X0 *= -1
            Y0 = gps2dist_azimuth(stla, lon0, lat0, lon0)[0] / 1000.0
            if stla < lat0:
                Y0 *= -1
...
if __name__ == "__main__":
    ...
    stlo, stla, phasemap, sncl_idx, stations, sncl_map = build_station_map(params)
    ...
    print(
        "             {:.3f}   {:.3f}   {:.3f}   {:.3f}".format(
            pTT.interp(0, 0), pTT.interp(10, 0), pTT.interp(0, 10), pTT.interp(10, 10)
        )
    )
    ...
    print(
        "             {:.3f}   {:.3f}   {:.3f}   {:.3f}".format(
            sTT.interp(0, 0), sTT.interp(10, 0), sTT.interp(0, 10), sTT.interp(10, 10)
        )
    )
